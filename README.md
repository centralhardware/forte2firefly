...
### Продвинутые возможности OCR

#### Кастомный Layout Analysis
- **Двухэтапное распознавание**: Основной OCR для всего чека + специализированный OCR только для MCC кода
- **Region-based OCR**: MCC код извлекается из нижней части чека (70-90% высоты) для повышения точности
- **Увеличение масштаба**: Регион с MCC кодом увеличивается в 3x для лучшего распознавания мелкого текста
- **Бинаризация изображения**: Применяется пороговая обработка для улучшения контраста
- **Digit-only Tesseract**: Отдельный экземпляр OCR настроен на распознавание только цифр (0-9) для MCC кодов



#### Preprocessing изображений
- Обрезка верхних 10% изображения (статус бар и UI элементы)
- Увеличение изображения в 2x с бикубической интерполяцией
- High-quality рендеринг с антиалиасингом

#### Настройки Tesseract
- **PSM 11**: Sparse text mode для основного OCR (находит текст в произвольном порядке)
- **PSM 7**: Single text line mode для MCC кода (оптимально для одной строки с числом)
- **OEM 1**: LSTM neural network engine для максимальной точности
- **Отключены словари**: Отключены системные словари для лучшего распознавания цифр и специальных символов
